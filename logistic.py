...
def cross_subtype1(train_X, train_y, val_test_X, val_test_y):
    # 划分验证集和测试集
    ...
    val_proba = best_model.predict_proba(val_X)[:, 1]

    val_metric = {
        'param': '',
        'accuracy': accuracy_score(val_y, val_pred),
        'auc': roc_auc_score(val_y, val_proba),
        'precision': precision_score(val_y, val_pred, zero_division=0),
